...
- Direction results include current-job recommendation metadata/lists, MIA metrics/ROC/anonymized score metadata, update-leakage candidates and anonymous per-client evidence, or aggregation baseline/attacked/defended metrics and aligned round/filter evidence. `/workbench/jobs` derives `key_metrics` from the matching direction.
