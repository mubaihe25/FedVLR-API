...
- `GET /workbench/jobs/{job_id}` returns `status`, `stage`, `progress`, `experiment_name`, timestamps, `direction`, `dataset`, `model`, execution fields, `failure_stage`, Chinese `error_summary`, expandable `error_detail`, runner/subprocess PID, return code, command, Python path, `cwd`, relative result/artifact pointers, `source`, and a config summary. Metadata values take precedence over runner-updated status timestamps.
