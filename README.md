...
`/workbench/*` powers the frontend attack-defense workbench validation and full-training job flow. It reads `FedVLR/configs/workbench_experiment_schema.json`, calls `FedVLR/scripts/generate_workbench_smoke_config.py` for normalized config generation, and starts only the whitelisted `FedVLR/scripts/run_workbench_smoke_job.py` runner. The 8 runtime performance parameters (`num_workers` / `prefetch_factor` / `pin_memory` / `persistent_workers` / `amp_enabled` / `cache_item_features_on_device` / `non_blocking_transfer` / `reuse_client_model_workspace`) are no longer part of the workbench experiment schema; they are backend-fixed safe defaults (`num_workers=0, prefetch_factor=None, pin_memory=false, persistent_workers=false, amp_enabled=false, cache_item_features_on_device=true, non_blocking_transfer=true, reuse_client_model_workspace=true`), stripped from `/workbench/options`, ignored/overridden in `/workbench/validate` and `/workbench/jobs` payloads, and never reach the FedVLR generator.

- `GET /workbench/options` returns canonical datasets, the eight launchable model choices, shared/fixed/direction/defense parameter groups, compatibility hints, parameter descriptors (excluding the 8 runtime performance parameters, which are backend-fixed), bounds, defaults, and Amazon target item options with Chinese display fields. FedVLR schema descriptors are the authoritative source for labels, ranges, steps, defaults, option labels, and dynamic robust limits.
- `POST /workbench/validate` validates and normalizes a workbench payload without writing a job, then runs `FedVLR/scripts/workbench_forward_preflight.py` in the FedVLR Python environment. The preflight loads the real dataset/model and must pass a minimal forward before validation succeeds; identical payloads may reuse a five-minute in-process cache. The payload is first coerced to the runtime-safe defaults before validation so old payloads that still carry the 8 fields cannot influence training.
- `POST /workbench/jobs` repeats the same validation/preflight gate (also with the runtime-safe-default override) before writing a job artifact under `FedVLR/outputs/workbench_jobs/{job_id}` and launching a restricted background full-training process with `subprocess.Popen`; invalid payloads return `launch_enabled=false` plus a clear `error_message`. New jobs persist the click-time `started_at` and canonical Chinese `experiment_name` in `metadata.json`.
- `GET /workbench/jobs?limit=12&page=1&direction=&dataset=&model=&source=&status=&date_from=&date_to=` lists workbench jobs from `FedVLR/outputs/workbench_jobs`, returning `job_id`, `experiment_name`, `direction`, `dataset`, `model`, `execution_mode`, `source`, `status`, timestamps, compact `key_metrics`, and relative result/artifact pointers. The archive excludes test records whose id or name contains `test` and records starting with `codex_`; legacy jobs may fall back from `started_at` to `created_at`, and remaining jobs are ordered by parsed start time descending.
- `GET /workbench/jobs/{job_id}` returns `status`, `stage`, `progress`, full `progress_detail`, `experiment_name`, timestamps, `direction`, `dataset`, `model`, execution fields, `failure_stage`, Chinese `error_summary`, expandable `error_detail`, actual/model-expected shape evidence, runner/subprocess PID, return code, command, Python path, `cwd`, relative result/artifact pointers, `source`, epoch metrics, GPU samples, performance summary, and a config summary. Metadata values take precedence over runner-updated status timestamps.
- `GET /workbench/jobs/{job_id}/logs?tail=200` returns the growing tail of `run.log` while training is active; a missing log file for an existing job returns an empty list.
...
- Progress is sourced from the runner's atomic `progress.json`; absent progress remains `null`/initializing rather than being synthesized. Completed jobs normalize to 100%, while failures retain their real failure phase and percentage. GPU CSV sampling is optional telemetry and missing samples do not change job status.
- `/result` normalizes new and legacy jobs into a stable response. New jobs expose `workbench-result-v2` with common training data plus current-job-only direction results; `partial` includes named missing evidence and failed jobs include the real phase and sanitized error detail.
- Direction results include current-job recommendation metadata/lists, MIA metrics/ROC/anonymized score metadata, update-leakage candidates and anonymous per-client evidence, or aggregation baseline/attacked/defended metrics and aligned round/filter evidence. Recommendation-manipulation responses pass through independent baseline/attack metrics, recommendations, ranks, Top50 hits, and attack-vs-baseline Jaccard; robust-defense jobs additionally pass through the corresponding `defense_metrics`, `defended_*` fields, and defense-vs-baseline Jaccard. No-defense jobs keep those defense fields absent. `/workbench/jobs` history previews keep attack and defended ranks/hits/Jaccards separate while final-stage summary fields may still describe `result_variant`.
- Item thumbnails use `/showcase/images/{datasetId}/{itemId}?size=thumb`. API responses and logs sanitize local absolute paths; large evidence blocks carry total/returned/truncated metadata where applicable.

Workbench jobs are restricted background processes, not a production queue. `/workbench/validate` and `/workbench/jobs` default missing execution mode to `full_train`; explicit legacy modes are rejected. Unsupported direction/model/dataset combinations fail validation with `field_errors` and are never downgraded. TopK is fixed to 50 regardless of a legacy payload value. Dynamic Krum/Bulyan `f` limits and TrimmedMean retained-client limits are derived from the sampled client count and rejected when invalid. Krum validates its four fields, Median validates `gradient_clip_norm` plus `outlier_strategy`, and robust preprocessing clipping remains independent from DP-style noise `max_grad_norm`. New jobs return `source=full_train` and preserve valid submitted parameters.

`model_dataset_execution` keeps `construct_verified`, `forward_verified`, `train_verified`, and `direction_verified` distinct. Construct-only records do not bypass the runtime preflight. Checkpoint fields are rejected until FedVLR exposes an explicit strict loader that can prove projection-shape compatibility.
